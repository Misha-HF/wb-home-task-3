...
def process_folder(result_path, folder_path, extensions_info):
    return diver(result_path, folder_path, extensions_info)

def process_folders_in_threads(results_path, folder_path, extensions_info):
    threads = []
    for element in folder_path.iterdir():
        if element.is_dir():
            thread = threading.Thread(target=process_folder, args=(results_path, element, extensions_info))
            thread.start()
...
def sorter(folder_platform_path):
    ...
    with tempfile.TemporaryDirectory() as tmp_platform_path:
        ...
        if process_folders_in_threads(tmp_path, folder_path, extensions_info) is False:
        ...
        shutil.copytree(str(tmp_path), str(results_path))
